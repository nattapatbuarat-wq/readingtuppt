...
const DB = (() => {
  ...
  const getNotifySettings = () => {
    ...
    levels.forEach(lv => {
      ...
      const classRooms = getClasses()
        .filter(c=>c.level===lv)
        .map(c=>({id:c.id,room:String(c.room),name:`${lv}/${c.room}`,lineWebhook:'',telegramWebhook:'',telegramChatId:''}));
      const merged = [...classRooms, ...savedRooms]
        .filter(r=>r && !hidden.includes(String(r.room)))
        .reduce((acc,r)=>{
          const key=String(r.room||r.name||r.id);
          acc[key]={id:r.id||`${lv}_${key}`,room:key,name:r.name||`${lv}/${key}`,lineWebhook:r.lineWebhook||'',telegramWebhook:r.telegramWebhook||'',telegramChatId:r.telegramChatId||''};
          return acc;
        },{});
      out[lv] = {
        lineWebhook: old.lineWebhook || '',
  ...
  const seedDefaults = () => {
    if(!ga('tea').length) {
      sa('tea',[{id:'T1',name:'ครูผู้สอน 1',password:'THTUPPT01'},{id:'T2',name:'ครูผู้สอน 2',password:'THTUPPT02'},{id:'T3',name:'ครูผู้สอน 3',password:'THTUPPT03'},{id:'T4',name:'ครูผู้สอน 4',password:'THTUPPT04'},{id:'T5',name:'ครูผู้สอน 5',password:'THTUPPT05'},{id:'T6',name:'ครูผู้สอน 6',password:'THTUPPT06'}]);
    }
    if(!ga('cls').length) {
      const cs=[];
...
// SIMULATE NOTIFICATION (LINE & TELEGRAM BY LEVEL 1-6)
async function triggerNotification(st, log) {
  ...
  const cfg = {
    lineWebhook: roomCfg.lineWebhook || levelCfg.lineWebhook || '',
    telegramWebhook: roomCfg.telegramWebhook || levelCfg.telegramWebhook || '',
    telegramChatId: roomCfg.telegramChatId || levelCfg.telegramChatId || ''
  };
  const msg = `📚 บันทึกรักการอ่านใหม่\nนักเรียน: ${st.name}\nชั้น: ${st.classLevel}/${st.classRoom} เลขที่ ${st.number||'-'}\nหนังสือ: ${log.bookTitle}\nวันที่อ่าน: ${fmtDate(log.readDate)}\nสถานะ: รอครูตรวจ`;
  const jobs = [];
  if(cfg.lineWebhook) jobs.push(fetch(cfg.lineWebhook,{method:'POST',mode:'no-cors',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,text:msg})}));
  if(cfg.telegramWebhook) {
    const body = cfg.telegramChatId ? {chat_id:cfg.telegramChatId,text:msg} : {text:msg};
    jobs.push(fetch(cfg.telegramWebhook,{method:'POST',mode:'no-cors',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}));
  }
  console.log(`[LINE/Telegram Notification ${lv}]`, msg);
  if(jobs.length){
    try { await Promise.allSettled(jobs); toast(`🔔 ส่งแจ้งเตือนกลุ่ม ${lv} แล้ว`, 'ok'); }
    catch { toast(`บันทึกแล้ว แต่ส่งแจ้งเตือนกลุ่ม ${lv} ไม่สำเร็จ`, 'er'); }
  } else {
    toast(`🔔 บันทึกแล้ว (ยังไม่ได้ตั้งค่า LINE/Telegram กลุ่ม ${lv})`, 'ok');
  }
}

// ฟังก์ชันสำหรับนำเข้าข้อมูลนักเรียนจากไฟล์ CSV
function importStudentsFromCSV(csvText) {
  ...
  lines.forEach(line => {
    // 1. ค้นหาบรรทัดที่เป็นหัวห้องเรียน เช่น "ชั้น ม. 4 ห้องที่ 1"
    const classMatch = line.match(/ชั้น\s*(ม\.\s*\d)\s*ห้องที่\s*(\d+)/);
    if (classMatch) {
      currentLevel = classMatch[1].replace(/\s+/g, ''); // แปลง "ม. 4" เป็น "ม.4"
      currentRoom = parseInt(classMatch[2]);
      
      // ค้นหาว่ามีห้องนี้ในระบบหรือยัง ถ้ายังให้สร้างใหม่
      let cls = DB.getClasses().find(c => c.level === currentLevel && c.room === currentRoom);
      if (!cls) {
        cls = { id: DB.gid(), level: currentLevel, room: currentRoom, name: `${currentLevel}/${currentRoom}` };
        DB.saveClass(cls);
      }
    ...
    // 2. ค้นหาบรรทัดที่เป็นข้อมูลนักเรียน (ต้องมีระดับชั้นและห้องก่อนถึงจะทำงาน)
    if (currentClassId) {
      const cols = line.split(',');
      if (cols.length >= 3) {
        const num = parseInt(cols[0].trim());
        const code = cols[1].trim();
        const name = cols[2].trim();

        // ตรวจสอบว่าเป็นบรรทัดรายชื่อจริงๆ (มีเลขที่, รหัส 5 หลัก และ ชื่อ)
        if (!isNaN(num) && code.length === 5 && !isNaN(code) && name.length > 0) {
          let st = DB.getStudentByCode(code) || { id: DB.gid(), studentCode: code };
          
          st.name = name;
          st.classId = currentClassId;
          st.classLevel = currentLevel;
          st.classRoom = currentRoom;
          st.number = num;
          st.academicYear = DB.getSettings().academicYear;
  ...
  toast(`✅ นำเข้าข้อมูลนักเรียนสำเร็จ ${importedCount} คน`, 'ok');
  Router.go('admin-students'); // รีเฟรชหน้า
}
  ...
  el.innerHTML = `
    ...
    <div class="sidebar-footer">
      ...
      <div style="font-size:11px; color:var(--gray-500); border-top:1px solid var(--gray-100); padding-top:5px; line-height:1.2;">
        สร้างโดย นายณัฐภัทร บัวราช ม.5/5
      </div>
    </div>`;
...
function pageHome() {
    ...
    <div class="home-wrap">
      ...
      <div class="home-cnt">
        ...
        <div style="margin-top: 2rem; color: rgba(255,255,255,0.7); font-size: 0.85rem; font-weight:500;">
          สร้างระบบโดย นายณัฐภัทร บัวราช มัธยมศึกษาปีที่ 5/5
        </div>
      </div>
    </div>`;
  document.getElementById('h-stu').onclick = () => Router.go('student-login');
  document.getElementById('h-tea').onclick = () => {
    modal('👩‍🏫 ครูเข้าสู่ระบบ',`<div class="fg"><label>รหัสผ่าน</label><input class="fi" type="password" id="tp" placeholder="THTUPPTXX"></div><div id="te" class="err" style="display:none"></div>`,()=>{
  ...
  document.getElementById('h-adm').onclick = () => {
    modal('⚙️ ผู้ดูแลระบบ',`<div class="fg"><label>รหัสผ่าน</label><input class="fi" type="password" id="ap" placeholder="THTUPPT"></div><div id="ae" class="err" style="display:none"></div>`,()=>{
  ...
  const app=document.getElementById('app'); app.className=''; app.innerHTML=`
    <div class="login-wrap">
      <div class="login-card">
        ...
        <div style="margin-top:1.5rem; font-size:11px; color:var(--gray-500)">สร้างโดย นายณัฐภัทร บัวราช ม.5/5</div>
      </div>
    </div>`;
...
function pageNotebook() {
  ...
  m.innerHTML = `
    ...
    <div class="sys-footer" style="margin-top:2rem; border-radius:var(--radius)">สร้างระบบโดย นายณัฐภัทร บัวราช มัธยมศึกษาปีที่ 5/5</div>`;
  loadLogs();
...
function openLogForm(logId=null) {
  ...
  modal(logId?'✏️ แก้ไขบันทึกการอ่าน':'➕ เพิ่มบันทึกรักการอ่านเล่มใหม่', html, () => {
    ...
    // TRIGGER NOTIFICATION SYSTEM FOR LINE & TELEGRAM GROUPS
...
function pageStudentProfile() {
  ...
  <div class="sys-footer" style="border-radius:var(--radius)">สร้างระบบโดย นายณัฐภัทร บัวราช มัธยมศึกษาปีที่ 5/5</div>`;
...
function doGrade(lid,status){
  ...
  const st=parseInt(document.getElementById('g-st')?.value)||3;
  const cm=document.getElementById('g-cm')?.value||'';
  const log=DB.getLog(lid); if(!log) return;
  log.status=status; DB.saveLog(log);
  const ex=DB.getScore(lid);
  DB.saveScore({id:ex?.id,logId:lid,studentId:log.studentId,points:Math.min(10,Math.max(0,pt)),stars:st,comment:cm});
...
function renderStuList(m){
  ...
  m.innerHTML=`
    <div class="sec-h">
      <h2 class="sec-t">👤 จัดการนักเรียน</h2>
      <div style="display:flex; gap:0.5rem;">
        <input type="file" id="csv-upload" accept=".csv" style="display:none">
        <button class="btn btn-outline" onclick="document.getElementById('csv-upload').click()">📥 นำเข้า CSV</button>
        <button class="btn btn-primary" onclick="openStuForm()">➕ เพิ่มนักเรียน</button>
      </div>
    </div>
  ...
  document.getElementById('ss').oninput=function(){ 
    document.getElementById('stu-tb').innerHTML=stuRows(DB.getStudents().filter(s=>s.studentCode.includes(this.value)||(s.name||'').toLowerCase().includes(this.value.toLowerCase()))); 
  };

  // ดักจับอีเวนต์เมื่อเลือกไฟล์ CSV
  setTimeout(() => {
    ...
    if (fileInput) {
      fileInput.onchange = e => {
        ...
        // อ่านไฟล์เป็น UTF-8 เพื่อให้ภาษาไทยไม่เพี้ยน
        reader.onload = ev => importStudentsFromCSV(ev.target.result);
...
function openTeaForm(tid=null){
  ...
  const html=`<div class="fg"><label>ชื่อครู *</label><input type="text" id="tf-n" class="fi" value="${esc(t?.name||'')}"></div><div class="fg"><label>รหัสผ่าน *</label><input type="text" id="tf-p" class="fi" value="${esc(t?.password||'')}" placeholder="THTUPPTXX"></div>`;
  ...
  return `<details class="notify-level">
    ...
    <div class="notify-body">
      <div class="notify-room" style="background:var(--purple-pale)">
        ...
        <div class="fg"><label>LINE Webhook URL</label><input type="url" class="fi notify-level-line" data-lv="${lv}" value="${esc(cfg.lineWebhook||'')}" placeholder="https://..."></div>
        <div class="fr">
          <div class="fg"><label>Telegram Bot API URL หรือ Webhook URL</label><input type="url" class="fi notify-level-tg" data-lv="${lv}" value="${esc(cfg.telegramWebhook||'')}" placeholder="https://api.telegram.org/bot.../sendMessage"></div>
          <div class="fg"><label>Telegram Chat ID</label><input type="text" class="fi notify-level-chat" data-lv="${lv}" value="${esc(cfg.telegramChatId||'')}" placeholder="-100xxxxxxxxxx"></div>
        </div>
      </div>
      ${rooms.length ? rooms.map(r=>`<div class="notify-room" data-lv="${lv}" data-room="${esc(r.room)}">
        <div class="notify-room-head">
          ...
          <button class="btn btn-sm btn-do" onclick="removeNotifyRoom('${encodeURIComponent(lv)}','${encodeURIComponent(r.room)}')">ลบห้อง</button>
        </div>
        <div class="fg"><label>LINE Webhook URL</label><input type="url" class="fi notify-room-line" value="${esc(r.lineWebhook||'')}" placeholder="https://..."></div>
        <div class="fr">
          <div class="fg"><label>Telegram Bot API URL หรือ Webhook URL</label><input type="url" class="fi notify-room-tg" value="${esc(r.telegramWebhook||'')}" placeholder="https://api.telegram.org/bot.../sendMessage"></div>
          <div class="fg"><label>Telegram Chat ID</label><input type="text" class="fi notify-room-chat" value="${esc(r.telegramChatId||'')}" placeholder="-100xxxxxxxxxx"></div>
        </div>
      </div>`).join('') : '<div class="empty" style="padding:1.2rem">ยังไม่มีห้องในระดับชั้นนี้</div>'}
      <div class="notify-actions"><button class="btn btn-outline btn-sm" onclick="addNotifyRoom('${encodeURIComponent(lv)}')">➕ เพิ่มห้องใน ${lv}</button></div>
    </div>
  </details>`;
}
...
function collectNotifySettingsFromForm(){
  ...
  document.querySelectorAll('.notify-level-line').forEach(i=>data[i.dataset.lv].lineWebhook=i.value.trim());
  document.querySelectorAll('.notify-level-tg').forEach(i=>data[i.dataset.lv].telegramWebhook=i.value.trim());
  document.querySelectorAll('.notify-level-chat').forEach(i=>data[i.dataset.lv].telegramChatId=i.value.trim());
  document.querySelectorAll('.notify-room').forEach(box=>{
    const lv=box.dataset.lv, room=box.dataset.room;
    if(!lv || !room) return;
    const cfg=data[lv];
    const item=(cfg.rooms||[]).find(r=>String(r.room)===String(room));
    if(!item) return;
    item.lineWebhook=box.querySelector('.notify-room-line')?.value.trim()||'';
    item.telegramWebhook=box.querySelector('.notify-room-tg')?.value.trim()||'';
    item.telegramChatId=box.querySelector('.notify-room-chat')?.value.trim()||'';
  });
  return data;
}
...
function addNotifyRoom(lv){
  ...
  modal(`➕ เพิ่มห้องใน ${lv}`, `<div class="fg"><label>เลขห้องหรือชื่อห้อง</label><input class="fi" id="new-notify-room" placeholder="เช่น 16 หรือ ห้องโครงการพิเศษ"></div><div id="new-notify-room-err" class="err" style="display:none"></div>`, ()=>{
    ...
    data[lv].rooms.push({id:DB.gid(),room:String(room),name:`${lv}/${room}`,lineWebhook:'',telegramWebhook:'',telegramChatId:''});
...
function pageAdminSettings() {
  ...
  <div class="pc"><h2 class="sec-t">🔔 ตั้งค่าแจ้งเตือน LINE / Telegram แยกกลุ่ม ม.1-ม.6</h2>
    <p style="color:var(--gray-500);font-size:.84rem;margin-bottom:1rem">กด Dropdown ของแต่ละระดับชั้นเพื่อจัดการห้อง เพิ่ม/ลบห้อง และตั้งค่า Webhook รายห้องได้เลย เมื่อนักเรียนส่งบันทึก ระบบจะส่งไปยังห้องของนักเรียนคนนั้นก่อน</p>
